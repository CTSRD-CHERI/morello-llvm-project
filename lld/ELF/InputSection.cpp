...
uint64_t InputSectionBase::getRelocTargetVA(const InputFile *file, RelType type,
  ...
  case R_MORELLO_RELAX_TGOT_TLS_GD_TO_IE_PAGE_PC:
    return getAArch64Page(c.got->getTgotAddr(sym) + a) - getAArch64Page(p);
  case R_MORELLO_TGOT_TLSDESC_PAGE:
    return getAArch64Page(c.got->getTgotTlsDescAddr(sym) + a) -
           getAArch64Page(p);
  ...
  case R_MORELLO_PLT_CAPFRAG_ADDEND: {
    uint64_t va = c.plt->getVA() - getMorelloExecBaseAddress(c);
