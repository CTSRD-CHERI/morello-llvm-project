...
void RelocationBaseSection::addSymbolReloc(
  ...
  // addendRelType in order to do so). Morello leaves it 0, however.
  // TODO: Stop doing this in the assembler and drop this hack
  bool writeZero = isCap && config->emachine != EM_AARCH64;
...
void RelocationBaseSection::partitionRels() {
  ...
  const std::optional<RelType> relativeCapRel = target->relativeCapRel;
