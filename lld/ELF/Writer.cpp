...
bool elf::isRelroSection(const OutputSection *sec) {
  ...
  if (cheriCapTable(c) && sec == cheriCapTable(c)->getParent()) {
...
SmallVector<PhdrEntry *, 0> Writer<ELFT>::createPhdrs(Partition &part) {
  ...
  Compartment *last_compartment = nullptr;
  for (OutputSection *sec : outputSections) {
    if (sec->partition != partNo || !needsPtLoad(sec))
      continue;
    if (sec->compartment == nullptr) {
      last_compartment = nullptr;
      continue;
    }
    if (sec->compartment != last_compartment) {
    ...
    last_compartment = sec->compartment;
  }
