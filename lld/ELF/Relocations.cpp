...
static void addPltEntry(PltSection &plt, GotPltSection &gotPlt,
  ...
  if (config->isCheriAbi && config->emachine != EM_AARCH64) {
    ...
    rel.addReloc({type, &gotPlt, sym.getGotPltOffset(gotPlt.compartment),
                  ...
                  sym, 0, R_ABS});
    addMorelloCapabilityFragment(&gotPlt, &sym,
                                 sym.getGotPltOffset(gotPlt.compartment), true);
  } else {
    gotPlt.addEntry(sym);
    rel.addReloc({type, &gotPlt, sym.getGotPltOffset(gotPlt.compartment),
                  sym.isPreemptible ? DynamicReloc::AgainstSymbol
...
static void addGotEntry(Compartment *c, Symbol &sym) {
  ...
  if (config->morelloC64Plt) {
    ...
    addMorelloC64GotRelocation(reltype, &sym, got(c), off, 0);
    return;
  }
...
static void addTpOffsetGotEntry(Compartment *c, Symbol &sym) {
  ...
  if (config->morelloC64Plt && !sym.isPreemptible)
    got(c)->relocations.push_back({R_SIZE, R_AARCH64_ABS64, off + 8, 0, &sym});
  ...
  auto compartFn = [](Compartment *c, Symbol &sym) {
    ...
    if (aux->needsTlsDesc) {
      ...
      if (config->morelloC64Plt && sym.isTls() && !sym.isPreemptible)
        got(c)->relocations.push_back({R_SIZE, R_AARCH64_ABS64,
            got(c)->getTlsDescOffset(sym) + 24, 0, &sym});
    }
    if (aux->needsTlsGd) {
