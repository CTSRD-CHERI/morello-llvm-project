...
bool elf::isDirectPcExpr(RelExpr expr) {
  return oneof<R_PC, R_AARCH64_PAGE_PC, R_ARM_PCA, R_MORELLO_VADREF,
...
template <class ELFT> static void addCopyRelSymbol(SharedSymbol &ss) {
  assert(!config->isCheriAbi && "CHERI does not permit copy relocations");
...
void RelocationScanner::processAux(RelExpr expr, RelType type, uint64_t offset,
  ...
  if (!config->shared) {
    ...
    if (sym.isObject() && !config->isCheriAbi) {
