...
static void addRelativeReloc(InputSectionBase &isec, uint64_t offsetInSec,
  ...
  if (expr == R_ABS_CAP)
    relativeType = *target->relativeCapRel;
  if (expr == R_ABS_CAP && target->relativeCapFuncRel &&
      type != target->symbolicCodeCapRel && sym.isFunc())
    relativeType = *target->relativeCapFuncRel;
...
static void addPltEntry(PltSection &plt, GotPltSection &gotPlt,
  ...
  if (config->isCheriAbi && config->emachine == EM_AARCH64)
    rel.addReloc({type, &gotPlt, sym.getGotPltOffset(c),
                  ...
                  sym, 0,
                  sym.isPreemptible ? R_MORELLO_PLT_CAPFRAG_ADDEND
...
static void addTpOffsetGotEntry(Compartment &c, Symbol &sym) {
  ...
  if (config->emachine == EM_AARCH64 && config->isCheriAbi && !sym.isPreemptible)
    c.got->relocations.push_back({R_SIZE, R_AARCH64_ABS64, off + 8, 0, &sym});
...
template <class ELFT, class RelTy> void RelocationScanner::scanOne(RelTy *&i) {
  ...
  // Undefined weak references will be static link time constants that do not
  // belong to any compartment for -no-pie static linking.
  if (config->isPic || !sym.isUndefWeak()) {
    if (expr == R_PLT_PC || sym.type == STT_FUNC || sym.type == STT_GNU_IFUNC)
      verifyExecSymbol(c, *sec, sym, offset);
    else
      verifyAccessSymbol(c, *sec, sym, offset);
  }
...
static bool handleCrossCompartmentCall(Compartment &c, Symbol &sym,
  ...
  RelType relativeType = target->relativeRel;
  if (target->relativeFuncRel)
    relativeType = *target->relativeFuncRel;
  if (config->isCheriAbi)
    relativeType = *target->relativeCapRel;
  if (config->isCheriAbi && target->relativeCapFuncRel)
    relativeType = *target->relativeCapFuncRel;
  sym.allocateAux(c);
  addPltEntry(*c.iplt, *c.igotPlt, *mainPart->relaDyn, relativeType, sym);
