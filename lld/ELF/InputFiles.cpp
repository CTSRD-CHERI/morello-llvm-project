...
static bool isCompatible(InputFile *file) {
  ...
  if (file->ekind == config->ekind && file->emachine == config->emachine) {
    if (config->emachine != EM_MIPS ||
      ...
      if (isCheriAbi(file) == config->isCheriAbi || isa<BitcodeFile>(file))
        return true;
      onlyCheriAbi = true;
    }
  }
...
static void readCheriVariants(const InputSection &sec,
  ...
  while (!contents.empty()) {
    ...
    if (contents.size() < header->getSize(sec.addralign)) {
      reportFatal(place, "data size too short. Expected: 0x" +
                             Twine::utohexstr(header->getSize(sec.addralign)) +
    ...
    contents = contents.drop_front(header->getSize(sec.addralign));
  }
}
