...
StackOffset AArch64FrameLowering::resolveFrameOffsetReference(
  ...
  if (isSVE) {
    ...
    FrameReg = RegInfo->hasBasePointer(MF)
                   ? RegInfo->getBaseRegister(MF)
                   : RegInfo->getStackPointerRegister(MF);
    return SPOffset;
  }
      ...
      case RegPairInfo::Cap:
        if (AArch64::CapRegClass.contains(NextReg) &&
            !invalidateRegisterPairing(RPI.Reg1, NextReg, IsWindows, NeedsWinCFI,
                                       NeedsFrameRecord, IsFirst, TRI, LR))
          RPI.Reg2 = NextReg;
        break;
...
void AArch64FrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  Register BasePointerReg =
      RegInfo->hasBasePointer(MF) ? RegInfo->getBaseRegister(MF) : Register();

  unsigned ExtraCSSpill = 0;

  // Figure out which callee-saved registers to save/restore.
  for (unsigned i = 0; CSRegs[i]; ++i) {
    const MCPhysReg Reg = CSRegs[i];

    // Add the base pointer register to SavedRegs if it is callee-save.
    if (BasePointerReg.isValid() &&
        RegInfo->isSuperOrSubRegisterEq(Reg, BasePointerReg))
      SavedRegs.set(Reg);
