...
@addr = addrspace(200) global ptr addrspace(200) blockaddress(@test_blockaddress, %block)
...
; CHECK-NEXT:    ret c30
  store volatile ptr addrspace(200) blockaddress(@test_blockaddress, %block), ptr addrspace(200) @addr
  %val = load volatile ptr addrspace(200), ptr addrspace(200) @addr
  indirectbr ptr addrspace(200) %val, [label %block]
...
; CHECK-LABEL: addr:
; CHECK-NEXT:	.chericap .Ltest_blockaddress$local+((.Ltmp0+1)-.Ltest_blockaddress$local)
