...
; RUN: llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 --relocation-model=pic < %s -o - | FileCheck %s --check-prefixes=CHECK,NOCODEPTR --allow-unused-prefixes
; RUN: llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 --relocation-model=pic < %s -o - --cheri-codeptr-relocs | FileCheck %s --check-prefixes=CHECK,CODEPTR
; RUN: llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 --relocation-model=pic < %s -o - -filetype=obj | llvm-readobj --relocs --symbols - | FileCheck %s --check-prefix=RELOCS
...
; RELOCS-NEXT: R_MORELLO_CAPINIT .L_Z8do_catchv$local 0x10
