...
; REQUIRES: asserts
; RUN: opt -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 -cheri-bound-allocas -o - -S %s -debug-only=cheri-bound-allocas 2>%t.dbg| FileCheck %s
; RUN: FileCheck %s -input-file=%t.dbg -check-prefix DBG
; RUN: llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 %s -o - | FileCheck %s -check-prefix ASM
...
; ASM-NEXT:    stp c0, c30, [csp, #0] // 16-byte Folded Spill
; ASM-NEXT:    bl cheribsdtest_dynamic_identity_cap
; ASM-NEXT:    ldr c1, [csp, #0]
; ASM-NEXT:    mov c2, c0
; ASM-NEXT:    mov c0, c1
; ASM-NEXT:    mov c1, c2
; ASM-NEXT:    bl cheribsdtest_check_cap_eq
