...
static bool isAArch64C64MapSymbol(StringRef Name) {
  return Name.starts_with_insensitive("$d.") ||
    Name.starts_with_insensitive("$c.") ||
    Name.starts_with_insensitive("$x.");
}
...
template <class ELFT> void GNUELFDumper<ELFT>::printFileHeaders() {
  ...
  if (e.e_machine == EM_MIPS)
    ...
    ElfFlags = printFlags(e.e_flags, ArrayRef(ElfHeaderAArch64Flags));
...
template <typename ELFT>
static std::optional<CHERINote>
getCHERINote(unsigned Machine, uint32_t NoteType, ArrayRef<uint8_t> Desc) {
  if (Desc.size() != 4)
    return std::nullopt;
...
template <class ELFT> void GNUELFDumper<ELFT>::printNotes() {
  ...
  auto ProcessNote = [&](const Elf_Note &Note, bool IsCore) -> Error {
    ...
    if (Name == "GNU") {
      ...
      if (std::optional<CHERINote> N =
              getCHERINote<ELFT>(Machine, Type, Descriptor)) {
        OS << "    " << N->Type << ": " << N->Value << '\n';
        return Error::success();
      }
    } else if (Name == "CORE") {
...
template <class ELFT> void LLVMELFDumper<ELFT>::printFileHeaders() {
  const Elf_Ehdr &E = this->Obj.getHeader();
  {
    ...
    if (E.e_machine == EM_MIPS)
      ...
      W.printFlags("Flags", E.e_flags, ArrayRef(ElfHeaderAArch64Flags));
