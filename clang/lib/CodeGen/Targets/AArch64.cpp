...
class AArch64ABIInfo : public ABIInfo, CHERICapClassifier {
  AArch64ABIKind Kind;
  bool hasPureCap;

public:
  AArch64ABIInfo(CodeGenTypes &CGT, AArch64ABIKind Kind, bool hasPureCap)
    : ABIInfo(CGT), CHERICapClassifier(CGT.getContext()), Kind(Kind),
...
public:
  AArch64TargetCodeGenInfo(CodeGenTypes &CGT, AArch64ABIKind Kind)
      : CommonCheriTargetCodeGenInfo(std::make_unique<AArch64ABIInfo>(
            CGT, Kind, CGT.getTarget().areAllPointersCapabilities())),
