...
AArch64BareMetalToolChain::getDefaultUnwindTableLevel(
...
static const char *getLinkerName(const Driver &D, const ArgList &Args) {
  if (Args.hasArg(options::OPT_fuse_ld_EQ) ) {
    ...
    if (Ld.ends_with_insensitive("lld"))
      return "ld.lld";
    if (Ld.ends_with_insensitive("bfd"))
...
void Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const toolchains::AArch64BareMetalToolChain &TC =
      static_cast<const toolchains::AArch64BareMetalToolChain &>(
          getToolChain());
  bool UsesLLD =
      Args.getLastArgValue(options::OPT_fuse_ld_EQ).ends_with_insensitive("lld") ||
    LinkerName == "ld.lld";
  ...
  if (!UsesLLD) {
    ...
    CmdArgs.push_back(Args.MakeArgString("--local-caprelocs=elf"));
  }
