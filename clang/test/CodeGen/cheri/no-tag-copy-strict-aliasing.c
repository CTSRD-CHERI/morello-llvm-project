...
void must_retain(long **p, long **q) {
  ...
  *(void (**)(long **, long **))(*p + 2) = &foo;
  memcpy(*q, *p, 32);
  // CHECK: @must_retain(i64 addrspace(200)* addrspace(200)* noundef [[P:%.*]], i64 addrspace(200)* addrspace(200)* noundef [[Q:%.*]]) addrspace(200)
...
void no_retain(long **q) {
  ...
  *(void (**)(long **, long **))(p + 2) = &foo;
  ...
  // CHECK: @no_retain(i64 addrspace(200)* addrspace(200)* noundef [[Q:%.*]]) addrspace(200)
  // CHECK:    call void @llvm.memcpy.p200i8.p200i8.i64(i8 addrspace(200)* align 8 {{%[0-9]+}}, i8 addrspace(200)* align 8 {{%[0-9]+}}
  // CHECK-SAME: , i64 32, i1 false){{$}}
...
void retain_char_array(long **q) {
  ...
  *(void (**)(long **, long **))(p + 16) = &foo;
  memcpy(*q, p, 32);
  // CHECK: @retain_char_array(i64 addrspace(200)* addrspace(200)* noundef [[Q:%.*]]) addrspace(200)
