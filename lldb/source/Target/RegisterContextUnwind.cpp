...
void RegisterContextUnwind::InitializeZerothFrame() {
  ...
  if (abi_sp && !abi_sp->GetFrameState(*reg_ctx_sp.get(), m_frame_state)) {
  ...
  if (m_cfa.GetAddress() == LLDB_INVALID_ADDRESS &&
      m_afa.GetAddress() == LLDB_INVALID_ADDRESS) {
    UnwindLogMsg(
...
void RegisterContextUnwind::InitializeNonZerothFrame() {
  ...
  // Determine the frame state.
  if (abi_sp && !abi_sp->GetFrameState(*this, m_frame_state)) {
    UnwindLogMsg("could not read frame state");
    return;
  }
...
bool RegisterContextUnwind::ReadFrameAddress(
  ...
  case UnwindPlan::Row::FAValue::isRegisterPlusOffset: {
    ...
    if (reg_info != nullptr && ReadRegister(reg_info, reg_value)) {
      if (abi_sp)
        reg_value = abi_sp->FixDataAddress(reg_value.GetAsUInt64());
