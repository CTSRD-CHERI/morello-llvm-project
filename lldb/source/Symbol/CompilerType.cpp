...
bool CompilerType::IsCHERICapabilityType() const {
  if (IsValid())
    if (auto type_system_sp = GetTypeSystem())
      return type_system_sp->IsCHERICapabilityType(m_type);
  return false;
}
...
CompilerType CompilerType::GetPointerType(AddressSpace address_space) const {
  if (IsValid()) {
    if (auto type_system_sp = GetTypeSystem())
      return type_system_sp->GetPointerType(m_type, address_space);
  }
  return CompilerType();
}
...
CompilerType::GetLValueReferenceType(AddressSpace address_space) const {
  if (IsValid())
    if (auto type_system_sp = GetTypeSystem())
      return type_system_sp->GetLValueReferenceType(m_type, address_space);
  return CompilerType();
}
...
CompilerType::GetRValueReferenceType(AddressSpace address_space) const {
  if (IsValid())
    if (auto type_system_sp = GetTypeSystem())
      return type_system_sp->GetRValueReferenceType(m_type, address_space);
  return CompilerType();
}
...
MemoryContentType CompilerType::GetMemoryContentType() const {
  if (IsValid())
    if (auto type_system_sp = GetTypeSystem())
      return type_system_sp->GetMemoryContentType(m_type);
